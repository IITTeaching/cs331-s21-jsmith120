...
class LinkedList:
    ...
    def cursor_insert(self, value):
        ...
        new node"""
        ### BEGIN SOLUTION
        var = self.cursor.next
        var2 = self.cursor.next
        newVal = self.Node(value)
        newVal.next = var2
        var2.prior = var
        newVal.prior = self.cursor
    ...
    def __str__(self):
        ...
        ### BEGIN SOLUTION
        if(self.head.next.val != None):
            temp = self.head.next.next
            s = "[" + str(self.head.next.val)
            while(temp.val != None):
                s += ", "
                s+= str(temp.val)
                temp = temp.next
    ...
    def insert(self, idx, value):
        ...
        for i in range(indx):
            temp = temp.next
        newVal = self.Node(value)
        newVal.prior = temp.prior
        newVal.next = temp
        temp.prior.next = newVal
        temp.prior = newVal
        self.length += 1
        ### END SOLUTION
        ...
        if(isinstance(other,LinkedList)):
            ...
            for i in range(lngth):
                ...
                if(temp2.val!=temp.val):
                    return False
                if(temp.val==None and temp2.val!=None):
                    return False
                if(temp.val!=None and temp2.val==None):
                    return False
            return True
