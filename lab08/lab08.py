...
def running_medians(iterable):
    ### BEGIN SOLUTION
    minV = Heap(lambda x:-x) 
    maxV = Heap(lambda x:x)
    lst = [iterable[0]]
    var = iterable[0]
    toAppend = iterable[0] 
    i=1
    while(i<len(iterable)):
        if(var>iterable[i]):
            if(len(maxV.data) > len(minV.data)):
                maxV.add(iterable[i])
                minV.add(var)
                temp = maxV.pop()
                var = temp
            ...
            if(len(maxV.data) < len(minV.data)):
                ...
                minV.add(iterable[i])
        if(len(maxV) > len(minV)):
            toAppend = (maxV.peek() + var) / 2
        elif(len(minV) > len(maxV)):
            toAppend = (minV.peek() + var) / 2
        else:
            toAppend = var
        lst.append(toAppend)
        i+=1  
