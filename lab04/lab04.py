...
class ArrayList:
    ...
    def append(self, value):
        ...
        if(self.len >= len(self.data)):
            lst = ConstrainedList((self.len)*2)
    ...
    def insert(self, idx, value):
        ...
        ### BEGIN SOLUTION
        lst = ConstrainedList((self.len)+1)
        if(self._normalize_idx((idx)) > len(self.data)):
            raise IndexError("invalid idx")
        i = 0
        while(i < self.len):   
            lst[i] = self.data[i]
            i+=1
    ...
    def __add__(self, other):
        ...
        ### BEGIN SOLUTION
        lst = ArrayList()
        lst.len = self.len + len(other)
        lst.data = ConstrainedList(self.len + len(other))
        i = 0
        while(i<self.len):
            lst.data[i] = self.data[i]
            i+=1
        j=0
        while(j<len(other)):
            lst.data[i]=other[j]
